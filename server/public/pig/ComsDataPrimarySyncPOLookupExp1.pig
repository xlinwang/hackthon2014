...
YY = FILTER Y BY data MATCHES 'com.ebay.mmp.coms.lookup.api.OrderLookupException.*unique constraint.*';
D = GROUP YY BY name;
E = FOREACH D GENERATE group, COUNT(YY.name), 'OrderLookupException' , 'Unique constraint violated : Error occured syncing';

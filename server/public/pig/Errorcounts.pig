...
C = FOREACH X GENERATE name,REGEX_EXTRACT(records, 'X-EBAY-API-SITEID=([0-9]+)', 1) as siteId,REGEX_EXTRACT(records, 'X-EBAY-API-COMPATIBILITY-LEVEL=([0-9]+)', 1) as version,REGEX_EXTRACT(records, 'X-EBAY-API-APP-NAME=([a-zA-Z0-9-:.]*)', 1) as appName;

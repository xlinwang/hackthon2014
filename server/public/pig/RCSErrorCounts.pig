...
ZZ = FILTER B BY name MATCHES 'ReviseCheckoutStatus';
X = FILTER ZZ BY (records MATCHES '.*APIError.*');
C = FOREACH X GENERATE name,REGEX_EXTRACT(records, 'X-EBAY-API-SITEID=([0-9]+)', 1) as siteId,CONCAT('APIError_',REGEX_EXTRACT(records, 'APIError_(\\d+)', 1)) as errorcode,REGEX_EXTRACT(records, '<PaymentType>(\\d+)</PaymentType>', 1) as updatetype;
D = GROUP C BY (name,siteId,errorcode,updatetype);
F = FOREACH D GENERATE FLATTEN(group) as (name,siteId,errorcode,updatetype), COUNT(C) as counts;
